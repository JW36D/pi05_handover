...
def test_smooth_overlap_weight_is_monotonic():
    from lerobot.async_inference.robot_client import RobotClient

    weights = [RobotClient._smooth_overlap_weight(i, 4) for i in range(4)]
...
def test_aggregate_action_queues_smooth_overlap_crossfades(robot_client):
    from lerobot.async_inference.helpers import TimedAction

    robot_client.latest_action = 4
    robot_client.action_queue = Queue()
    for timestep in (5, 6, 7, 8):
        robot_client.action_queue.put(
            TimedAction(
                action=torch.full((6,), 10.0, dtype=torch.float32),
                timestep=timestep,
                timestamp=time.time(),
            )
        )
    ...
    incoming = [
        TimedAction(
            action=torch.full((6,), 20.0 + idx, dtype=torch.float32),
            timestep=timestep,
            timestamp=time.time(),
        )
        for idx, timestep in enumerate((5, 6, 7, 8))
    ]

    robot_client.config.aggregate_fn_name = "smooth_overlap"
    robot_client._aggregate_action_queues(incoming)

    actions = [a.get_action()[0].item() for a in robot_client.action_queue.queue]
    assert actions[0] < actions[1] < actions[2] < actions[3]
    assert actions[0] > 10.0
    assert actions[-1] < 28.0
